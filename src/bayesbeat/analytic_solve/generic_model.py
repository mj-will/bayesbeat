...
import sympy
from sympy import expand, factor, Function, Rational, collect, factorial, simplify
from sympy import symbols, latex, multiline_latex, Poly, powsimp, cancel, lambdify
...
omega_1, A_1, theta_1, phi_1, tau_1 = symbols('omega_1 A_1 theta_1 phi_1 tau_1', real=True)
omega_2, A_2, theta_2, phi_2, tau_2 = symbols('omega_2 A_2 theta_2 phi_2 tau_2', real=True)
...
E_1, E_2 = symbols('B_1 B_2', real=True)        # The exponential bit of the decaying amplitude if needed
...
print()
print('Beam position mu expressed as a combination of sines (in exponential expansion form)')
...
for p in range(0,g):
    if mu_coef[mu**p] != 0:                         # Given a power of mu (mu**p) where the coefficient exists
        b_p1 = (mu**p).subs(mu, mu_1 + mu_2 + x_0)  # create an expression with the mu**p with actual position subbed in.
    ...
    print(C_x[p], ': Expansion complete')           # Simple indicator of how far through the expansion we are
...
d_3_I = expand(d_1 * I*Rational(0.5)*(sympy.exp(I*(w_1+w_2)/2) - sympy.exp(-I*(w_1+w_2)/2)))    # Sine
d_3_Q = expand(d_1 *   Rational(0.5)*(sympy.exp(I*(w_1+w_2)/2) + sympy.exp(-I*(w_1+w_2)/2)))    # Cosine
...
for p in range(1,g):
    All_terms[p] = AC_terms.coeff(sympy.cos(p*w_1-p*w_2)) * sympy.cos(p*w_1-p*w_2)
